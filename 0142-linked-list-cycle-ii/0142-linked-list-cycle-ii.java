...
// Approach : I use two pointers fast and slow. Fast moves twice the speed of slow. When they meet, slow has covered distance till start of cycle (i) + some distance in the cycle (j), whereas fast has covered twice of it, which is also equal to distance till start of cycle (i) + n cycles + some distance in the cycle (j). -> i + nx + j
// Also, fast covered twice distance, so nx+i+j = 2i+2j. So, nx=i+j => nx-j=i. 
// Fast is currently at nx+j inside the cycle, moving slow to 0 and moving it ahead till it meats fast will give us the start of the cycle. This is because, slow will move i steps, and after i steps, fast will be at nx+j+i = 
// nx+j+nx-j=2nx. Basically 2x cycles without any additional steps -> this is the beginning of the cycle.
